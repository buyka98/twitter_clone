...
class TweetTranslation extends StatelessWidget {
  ...
  const TweetTranslation(
      {Key? key, required this.tweetKey, required this.description, required this.languageCode, this.textStyle, this.urlStyle})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    try {
      if (/*languageCode == null ||*/ languageCode == localeLanguageCode) return SizedBox.shrink();

      if (state.tweetsTranslations.containsKey(tweetKey)) {
        if (state.tweetsTranslations[tweetKey] != null) {
          return _translation(state.tweetsTranslations[tweetKey]!, context, textStyle, urlStyle);
        } else {
...
Widget _translation(Translation translation, BuildContext context, TextStyle? textStyle, TextStyle? urlStyle) {
  return Column(
            ...
            Row(
              children: [
                Text(
                  ...
                  style: TextStyle(fontSize: 11, color: AppColor.lightGrey, fontWeight: FontWeight.bold),
                ),
              ],
            ),
          ],
