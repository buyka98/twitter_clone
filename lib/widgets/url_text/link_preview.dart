...
import 'package:link_preview_generator/src/utils/analyzer.dart' show LinkPreviewAnalyzer;
...
class LinkPreviewer extends StatelessWidget {
  ...
  String? getUrl() {
    ...
    RegExp reg = RegExp(r"(https?|http)://[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]*");
  ...
  Widget build(BuildContext context) {
    ...
    if (state.linkWebInfos.containsKey(uri)) return _buildLinkPreview(state.linkWebInfos[uri]!, uri, Theme.of(context));
    return FutureBuilder(
        builder: (BuildContext context, AsyncSnapshot<InfoBase?> snapshot) {
          if (snapshot.hasData) {
            state.addWebInfo(uri, snapshot.data! as WebInfo);
            return _buildLinkPreview(snapshot.data! as WebInfo, uri, Theme.of(context));
          }
        ...
        future: LinkPreviewAnalyzer.getInfo(uri, cacheDuration: Duration(hours: 24), multimedia: true));
  }
  ...
  Widget _buildLinkPreview(WebInfo info, String uri, ThemeData theme) {
    ...
    if (!LinkPreviewAnalyzer.isNotEmpty(info.title) && LinkPreviewAnalyzer.isNotEmpty(info.image)) {
          ...
          children: <Widget>[
            ...
            if (LinkPreviewAnalyzer.isNotEmpty(info.title))
              Padding(
                ...
                child: Text(
                  ...
                  style: TextStyles.titleStyle.copyWith(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.black87),
                ),
              ),
            ...
            Padding(
              ...
              child: Text(
                ...
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
                style: TextStyles.subtitleStyle.copyWith(fontWeight: FontWeight.w400, fontSize: info.title.isNotEmpty ? 14 : 16),
              ),
            ),
          ],
        ),
