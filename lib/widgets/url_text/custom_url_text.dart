    ...
    for (var result in resultMatches) {
      if (result.isUrl) {
        widgets.add(_LinkTextSpan(
            onHashTagPressed: onHashTagPressed, text: result.text, style: urlStyle ?? const TextStyle(color: Colors.blue)));
      } else {
        widgets.add(TextSpan(text: result.text, style: style ?? const TextStyle(color: Colors.black)));
      }
    }
    return widgets;
  }
...
class _LinkTextSpan extends TextSpan {
  final Function(String)? onHashTagPressed;
  _LinkTextSpan({required TextStyle style, required String text, this.onHashTagPressed})
      : super(
            style: style,
            text: text,
