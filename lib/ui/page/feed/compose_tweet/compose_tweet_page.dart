import 'dart:io';
import 'package:bwitter/ui/page/feed/compose_tweet/widget/compose_bottom_icon_widget.dart';
import 'package:bwitter/ui/page/feed/compose_tweet/widget/compose_tweet_image.dart';
import 'package:bwitter/ui/page/feed/compose_tweet/widget/widget_view.dart';
import 'package:flutter/material.dart';
import 'package:flutter/rendering.dart';
import 'package:bwitter/helper/constants.dart';
...
class ComposeTweetPage extends StatefulWidget {
  const ComposeTweetPage({Key? key, required this.isRetweet, this.isTweet = true}) : super(key: key);
...
class _ComposeTweetReplyPageState extends State<ComposeTweetPage> {
  ...
  _scrollListener() {
    if (scrollController.position.userScrollDirection == ScrollDirection.reverse) {
      if (!isScrollingDown) {
        Provider.of<ComposeTweetState>(context, listen: false).setIsScrollingDown = true;
      }
    }
    if (scrollController.position.userScrollDirection == ScrollDirection.forward) {
      Provider.of<ComposeTweetState>(context, listen: false).setIsScrollingDown = false;
    }
  }
      ...
      body: Stack(
        ...
        children: <Widget>[
          SingleChildScrollView(
            ...
            child: widget.isRetweet ? _ComposeRetweet(this) : _ComposeTweet(this),
          ),
          Align(
...
class _ComposeRetweet extends WidgetView<ComposeTweetPage, _ComposeTweetReplyPageState> {
                ...
                children: <Widget>[
                  ...
                  ConstrainedBox(
                    ...
                    child: TitleText(model.user!.displayName!,
                        fontSize: 16, fontWeight: FontWeight.w800, overflow: TextOverflow.ellipsis),
                  ),
...
class _ComposeTweet extends WidgetView<ComposeTweetPage, _ComposeTweetReplyPageState> {
        ...
        Stack(
          children: <Widget>[
            ...
            Row(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: <Widget>[
                // CircularImage(path: viewState.model!.user!.profilePic, height: 40),
                const SizedBox(width: 10),
                ConstrainedBox(
                  constraints: BoxConstraints(minWidth: 0, maxWidth: MediaQuery.of(context).size.width * .5),
                ...
                customText('${viewState.model!.user!.userName}', style: TextStyles.userNameStyle.copyWith(fontSize: 15)),
                const SizedBox(width: 5),
                Padding(
                  padding: const EdgeInsets.only(top: 3),
                  child: customText('- ${Utility.getChatTime(viewState.model!.createdAt)}',
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          viewState.widget.isTweet ? const SizedBox.shrink() : _tweetCard(context),
          Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: <Widget>[
              ...
              Expanded(
                child: _TextField(
                  isTweet: viewState.widget.isTweet,
                  textEditingController: viewState._textEditingController,
                ),
              )
            ],
...
class _TextField extends StatelessWidget {
  const _TextField({Key? key, required this.textEditingController, this.isTweet = false, this.isRetweet = false})
      : super(key: key);
                ...
                return _UserTile(
                  ...
                  onUserSelected: (user) {
                    textEditingController.text =
                        Provider.of<ComposeTweetState>(context, listen: false).getDescription(user.userName!) + " ";
