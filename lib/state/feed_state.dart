    ...
    if (!isBusy && feedList != null && feedList!.isNotEmpty) {
      list = feedList!.where((x) {
        ...
        if (x.user!.userId == userModel.userId ||
            (userModel.followingList != null && userModel.followingList!.contains(x.user!.userId))) {
          return true;
        } else {
          return false;
        }
  ...
  Future<String?> uploadFile(File file) async {
    try {
      ...
      var storageReference =
          FirebaseStorage.instance.ref().child("tweetImage").child(path.basename(DateTime.now().toIso8601String() + file.path));
